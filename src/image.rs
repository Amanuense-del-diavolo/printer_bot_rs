...
pub fn render_image(file_path: &str, settings: &Settings) -> Result<Vec<[u8; 90]>, PrinterBotError> {
    ...
    debug!("Get printer status");
    let mut printer = driver::PrinterCommander::main("/dev/usb/lp0")?;
    debug!("Got printer status");
    let status = printer.read_status()?;
    trace!("{:#?}", status);
    debug!("Status assigned");
    debug!("Tape width {:?}", status.pixel_width());
